...
int main(int argc, char** argv){
    if(argc < 3 || argc > 3){
        std::cout << "Uso: ./ejercicio2 <ipv4> <port>" << std::endl;
        return -1;
    }
    ...
    while(buffer[0] != 'q'){
        ...
        int bread = recvfrom(sock, (void*)buffer, 2, 0, &cliente, &clienteLength);
        if(bread < 0){
        ...
        std::cout << bread << " bytes de " << host << ":" << serv << std::endl;
        ...
        if(sendToSize > 0){
            ...
            int err = sendto(sock, sendToBuffer, sendToSize, 0, &cliente, clienteLength);
            if(err < 0){
